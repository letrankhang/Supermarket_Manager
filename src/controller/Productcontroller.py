...
from src.gui.tabs.products_ui import Ui_ProductsTab
...
from src.utils.Theme import set_badge

logger = logging.getLogger(__name__)

_PAGE_SIZE = 10
_MAX_VISIBLE_PAGE_BUTTONS = 3  
...
class ProductController(QWidget, Ui_ProductsTab):
    def __init__(self, parent=None) -> None:
        ...
        add_awesome_left_icon(self.edtSearch, "search")
    ...
    def _setup_table_columns(self) -> None:
        ...
        self.tblProducts.setColumnWidth(_COL_BARCODE, 160)  
        
        header.setSectionResizeMode(_COL_CATEGORY, QHeaderView.ResizeMode.Interactive)
        self.tblProducts.setColumnWidth(_COL_CATEGORY, _CATEGORY_COLUMN_WIDTH)
        header.setSectionResizeMode(_COL_NAME, QHeaderView.ResizeMode.Stretch)

        self.tblProducts.setWordWrap(False)
        self.tblProducts.setTextElideMode(Qt.TextElideMode.ElideRight)


    def _setup_events(self) -> None:
        self.btnAdd.clicked.connect(self._open_create_dialog)
        self.btnAddCategory.clicked.connect(self._open_create_category_dialog)
        self.edtSearch.returnPressed.connect(self._apply_filters)
        self.cboCategory.currentIndexChanged.connect(self._apply_filters)
        self.btnEdit.clicked.connect(self._edit_selected_product)
        self.btnDelete.clicked.connect(self._delete_selected_product)
    ...
    def _render_category_filter(self, categories: List[CategoryDTO]) -> None:
        self.cboCategory.blockSignals(True)
        while self.cboCategory.count() > 1:
            self.cboCategory.removeItem(1)
        for category in categories:
            self.cboCategory.addItem(category.category_name, category.category_id)
        self.cboCategory.blockSignals(False)
    ...
    def _render_current_page(self) -> None:
        ...
        self.tblProducts.setRowCount(len(page_items))
    ...
    def _render_row(self, row: int, p: ProductDTO) -> None:
        ...
        for col, value in text_values.items():
            ...
            self.tblProducts.setItem(row, col, item)

        status_text = self._compute_status(p)
        lbl_status = QLabel(status_text, self.tblProducts)
        lbl_status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        set_badge(lbl_status, _STATUS_BADGE.get(status_text, "success"))
        status_container = QWidget(self.tblProducts)
        status_layout = QHBoxLayout(status_container)
        status_layout.setContentsMargins(4, 4, 4, 4)
        status_layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        status_layout.addWidget(lbl_status)
        self.tblProducts.setCellWidget(row, _COL_STATUS, status_container)
    ...
    def _get_selected_product_id(self) -> Optional[int]:
        ...
        if row < 0:
            return None
        item = self.tblProducts.item(row, _COL_BARCODE)
        if item is None:
            return None
